...
def registra_log(
    arquivo, nomeProjeto, fonteDado, nomEtapa, nomStatus, dscMsgProcessamento, paramJson
):
    v_arquivo = arquivo
...
def consulta_fonte_dado(nomArquivo, idProjeto, nomeFonteDado, paramJson, nomEtapa):
    ...
    try:
        ...
        v_dscMsgProcessamento = (
            "5 - Etapa de Verificar se a Fonte de Dados está Cadastrada."
        )
...
def importa_projeto(arquivo, aba, nomEtapa, paramJson):
    ...
    try:
        ...
        idProjeto = consulta_projeto(v_arquivo, nomeProjeto, paramJson, nomEtapa)
        ...
        idProjeto = consulta_projeto(v_arquivo, nomeProjeto, paramJson, v_nomEtapa)
...
def importa_fonte_dado(arquivo, nomeProjeto, aba, paramJson, nomEtapa):
    ...
    try:
        ...
        idProjeto = consulta_projeto(v_arquivo, nomeProjeto, v_paramJson, nomEtapa)

        nomEtapa = "5 - Etapa de Consulta Fonte Dados"

        idFonteDados = consulta_fonte_dado(
            v_arquivo, idProjeto, nomeFonteDados, v_paramJson, nomEtapa
        )
        ...
        if idFonteDados == 1:
            ...
            v_dscMsgProcessamento = (
                "6 - Fonte de Dados já está cadastrado na base de dados."
            )
...
def consulta_estrutura_fonte_dado(
    arquivo, abaEstruturaFonteDado, idProjeto, idFonteDado, paramJson, nomEtapa
):
    v_arquivo = arquivo
    ...
    df = pd.read_excel(v_arquivo, sheet_name=v_abaEstruturaFonteDado)
    ...
    try:
        ...
        if dfNomProjeto.empty:
            ...
            v_dscMsgProcessamento = (
                "8 - Não foi encontrado o projeto cadastrado na base de Data Quality."
            )
            ...
            IdEstruturaFonteDados = dfFonteDados["IdEstruturaFonteDados"].to_string(
                index=False
            )
            ...
            if IdEstruturaFonteDados != "0":
                print(
                    "Etapa de confirmação do cadastro da Estrutura da Fonte de Dados."
                )
...
def importa_estrutura_fonte_dado(
    arquivo, aba, paramJson, idProjeto, idFonteDado, IdEstruturaFonteDados, nomEtapa
):
    v_arquivo = arquivo
    v_aba = aba
    v_paramJson = paramJson
    ...
    try:
        ...
        if v_IdEstruturaFonteDados != "0":
            ...
            v_dscMsgProcessamento = (
                "A estrutura do arquivo já está cadastrado em nossa base de dados!"
            )
            ...
            for index, rowEstruturaFonteDado in df.iterrows():
                ...
                cur.execute(
                    """INSERT INTO [dq].[TB_ESTRUTURA_FONTE_DADO](
                                  [IdProjeto]
                                 ,[IdFonteDados]
                                 ,[NomFonteDados]
                                 ,[NomColuna]
            ...
            IdEstruturaFonteDados = consulta_estrutura_fonte_dado(
                v_arquivo, v_aba, v_idProjeto, v_idFonteDado, v_paramJson, nomEtapa
            )
        ...
        print(
            "importa_estrutura_fonte_dado - Error: Failed to insert data into the database:",
            e,
        )
...
def importa_planilha_dq(
    arquivo, abaProjeto, abaFonteDado, abaEstruturaFonteDado, paramJson
):
    v_arquivo = arquivo
    ...
    idProjeto, nomeProjeto = importa_projeto(
        v_arquivo, v_abaProjeto, "2 - Etapa de Cadastro de Projeto", v_paramJson
    )

    idFonteDados = importa_fonte_dado(
        v_arquivo,
        nomeProjeto,
        v_abaFonteDado,
        v_paramJson,
